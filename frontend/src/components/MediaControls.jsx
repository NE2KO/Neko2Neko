...
export default function MediaControls({
  ...
  const updateTime = useCallback(() => {
    const media = mediaRef.current;
    if (!media || isSeekingRef.current || media.paused) return;
  ...
  useEffect(() => {
    ...
    const skip = useCallback((seconds) => {
      ...
      if (media.duration) {
        if (progressFillRef.current) {
          progressFillRef.current.style.width = `${(media.currentTime / media.duration) * 100}%`;
        }
        if (currentTimeRef.current) {
    ...
    const handleAdvance = useCallback((autoEnded = false, isManual = false) => {
      ...
      if (shuffle) {
        const pool = folderFiles.filter(f => f.id !== currentFile.id);
        const next = pool.length ? pool[Math.floor(Math.random() * pool.length)] : folderFiles[0];
    ...
    }, [handleAdvance, mediaRef, onNext, playlistMode, storeNext]);
    ...
    const handlePlayPrevious = useCallback(() => {
      ...
      if (shuffle) {
        const pool = folderFiles.filter(f => f.id !== currentFile.id);
        const prev = pool.length ? pool[Math.floor(Math.random() * pool.length)] : folderFiles[0];
    ...
    }, [onFileChange, onPrevious, playlistMode, storePrevious]);
    ...
    useEffect(() => {
      ...
      const onEnded = () => handleAdvance(true);
    ...
    }, [mediaRef, handleAdvance, updateTime, currentFile?.id, type]);
    ...
    return (
      ...
      <button onClick={handlePlayPrevious} className="p-2 text-white hover:bg-white/10 rounded-full transition-all active:scale-75">
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
      <path d="M15 17l-5-5 5-5" />
      </svg>
      </button>

      <button onClick={() => skip(-5)} className="p-2 text-white hover:bg-white/10 rounded-full transition-all active:scale-75">
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
      ...
      className="w-14 h-14 bg-white text-black rounded-full flex items-center justify-center shadow-lg hover:scale-105 active:scale-90 transition-all duration-200"
      >
      {isPlaying ? (
      ...
      <button onClick={handlePlayNext} className="p-2 text-white hover:bg-white/10 rounded-full transition-all active:scale-75">
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
